...
const AIRING_REFRESH_INTERVAL_MS = 60 * 1000;
...
export const createAiringDashboardController = ({
      ...
      refreshHandle = window.setInterval(() => {
        ...
        render({
          ...buildAiringDashboardModel({
            entries: currentModel.__entries,
            animeItems: currentModel.__animeItems,
            scheduleMap: currentModel.__scheduleMap,
            locale: currentModel.__locale,
            timeZone: currentModel.__timeZone,
            nowMs: Date.now()
          }),
          __entries: currentModel.__entries,
          __animeItems: currentModel.__animeItems,
          __scheduleMap: currentModel.__scheduleMap,
          __locale: currentModel.__locale,
          __timeZone: currentModel.__timeZone
        });
      }, AIRING_REFRESH_INTERVAL_MS);
    }
  };
  ...
  return {
    ...
    async update({ entries, animeItems, locale, timeZone } = {}) {
      ...
      const scheduleMap = await fetchAiringSchedules(normalizedAnimeItems);
      ...
      render({
        ...model,
        __entries: normalizedEntries,
        __animeItems: normalizedAnimeItems,
        __scheduleMap: scheduleMap,
        __locale: locale,
        __timeZone: timeZone
      });
    },

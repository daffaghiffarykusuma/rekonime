...
- `js/airing-schedule.js`: AniList airing schedule fetch/cache, dashboard model building, countdown formatting, and refresh ticks.
- `js/airing-dashboard.js`: renderer adapter for home + watchlist airing dashboard sections.
...
- `js/services/*`: API client, cache, rate limiter, schema validation, analytics, error handling, logging, and catalog payload normalization.
...
- Airing dashboard: watchlist `planned` and `watching` entries pass through the Airing Schedule module, which queries live schedule metadata, caches it locally, and returns local-time next-episode countdown models for `/` and `/watchlist`.
